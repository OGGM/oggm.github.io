...
<i> Fig. 2: Percentage of glaciers reaching “full success” in the dynamical spinup calibration. 
 [Notebook that created the figure and includes additional analysis](https://nbviewer.org/urls/cluster.klima.uni-bremen.de/~oggm/oggm-standard-projections/analysis_notebooks/1.6.3_2025_6_gdirs_proj_comparisons/0_error_analysis_gdirs_oggm_v16_2025.6.ipynb)</i>
<br><br>

All preprocessed glacier directories apply the dynamical spinup, which ensures that OGGM matches the glacier area at the RGI date and the glacier mass balance during the 2000–2020 simulation, taking elevation feedbacks into account. If a glacier reaches “full success” in the dynamical calibration, this means that a melt factor can be found that matches the observed geodetic mass balance within 20% of the error estimate provided by Hugonnet et al. (2021).
...
<i> Fig. 3: Glacier projection differences in 2000 and 2100 relative to the OGGM v1.6.3 Standard Projections for different OGGM v1.6 preprocessed glacier directories. Numbers are given in % relative to the 2000 OGGM v1.6.3 Standard Projections. We used only one GCM (MRI-ESM2-M) and one scenario (SSP1-2.6) for the comparison and aggregated all glaciers, even though some options contain more failing glaciers than others. Restricting the analysis to “commonly running glaciers” leads to differences of <0.5% in all regions except the Low Latitudes (region 16), where differences are around 1%. For comparison, we also include the Farinotti et al. (2019) community volume estimate, which is valid at the RGI62 inventory date. [Link to figure with volume projections from 2000 to 2100, including GloGEM and PyGEM](https://cluster.klima.uni-bremen.de/~oggm/oggm-standard-projections/analysis_notebooks/1.6.3_2025_6_gdirs_proj_comparisons/figures/3_proj_all_options_plus_glogem_oggm_ssp126_one_gcm.png). [Link to notebook used for the analysis](https://nbviewer.org/urls/cluster.klima.uni-bremen.de/~oggm/oggm-standard-projections/analysis_notebooks/1.6.3_2025_6_gdirs_proj_comparisons/3_volume_projection_differences.ipynb). </i>
<br><br>
...
### Discussion
- Should I already use the RGI70G preprocessed gdir? 
